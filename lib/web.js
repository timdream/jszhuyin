...
JSZhuyinWebIME.prototype.handleEvent = function(evt) {
  ...
  if (evt.type === 'keydown') {
    ...
    var candidateSelection = (evt.shiftKey &&
                              evt.keyCode >= 49 &&
                              evt.keyCode <= 57);

    if (!specialKeys && !candidateSelection)
  ...
  if (evt.type === 'keydown' && shiftKey && code >= 49 && code <= 57) {
    var n = code - 49;
    var handled = this.candidatesList.selectCandidate(n);
