...
JSZhuyinWebIME.prototype.handleKeyEvent = function(code) {
  var handled =
    this.jszhuyin.handleKeyEvent(code, ++this.requestId);
...
JSZhuyinWebIME.prototype.handleEvent = function(evt) {
  ...
  var symbolCode = JSZhuyinLayoutMapper.getSymbolCodeFromCode(code);

  var handled = this.handleKeyEvent(symbolCode);
  if (handled) {
    evt.preventDefault();
  } else {
    if (symbolCode !== code) {
