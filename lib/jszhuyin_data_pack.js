...
JSZhuyinDataPack.prototype.getFirstResult = function() {
  ...
  var length = this.packed[Float32Encoder.STRING_LENGTH];

  return {
    'str': this.packed.substr(Float32Encoder.STRING_LENGTH + 1, length),
...
JSZhuyinDataPack.prototype.unpack = function() {
  ...
  var unpacked = [];
  var length = parseInt(this.packed[Float32Encoder.STRING_LENGTH]);

  unpacked.push(this.getFirstResult());

  var i = Float32Encoder.STRING_LENGTH + 1 + length;
  while (i < this.packed.length) {
    var str = this.packed.substr(i, length).replace('#', '');
    unpacked.push({ 'str': str });
...
JSZhuyinDataPack.prototype.pack = function() {
  ...
  var packed = [
    Float32Encoder.encode(this.getFirstResultScore()),
    length.toString(10)];
