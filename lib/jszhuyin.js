...
function(queryData) {
  ...
  // The |startsWithCompositions| array contains JSZhuyinCachedResult instances
  ...
  var lastCachedResult;
  var i = 0;
  var n = expendedEncodedSounds.length;
  var slicedExpendedEncodedSounds;
  // This loop will ensure startsWithCompositions is populated with
  // results from length = 1 to n - 1.
  while (++i < n) {
    slicedExpendedEncodedSounds = expendedEncodedSounds.slice(0, i);
    lastCachedResult = this.cache.get(slicedExpendedEncodedSounds);
    if (!lastCachedResult) {
    ...
    startsWithCompositions.push([i, lastCachedResult]);
  }

  // Get the composed result data of the full length expended encoded sounds.
  // With the |startsWithCompositions| the previous loop have populated.
  // It too has too be put into the cache even if the result is null.
  lastComposedResultData =
    this._getFirstComposedResult(expendedEncodedSounds, startsWithCompositions);
  this.addToCache(slicedExpendedEncodedSounds, lastComposedResultData);
