...
function(queryData) {
  ...
  while (++i < n) {
    ...
    previousFirstResults[i] = lastCachedResult;
  }

  // Get the composed result data of the full length expended encoded sounds.
  // With the |previousFirstResults| the previous loop have populated.
  // It too has too be put into the cache even if the result is null.
  lastComposedResultData =
    this._getFirstComposedResult(expendedEncodedSounds, previousFirstResults);
  this.addToCache(slicedExpendedEncodedSounds, lastComposedResultData);
...
function(expendedEncodedSounds, previousFirstResults) {
  ...
  var compositions = [];
  var combinations =
    BopomofoEncoder.getSymbolCombinations(expendedEncodedSounds);
  combinations.forEach(function(symbolCodes) {
    if (symbolCodes.length > this.LONGEST_PHRASE_LENGTH) {
      return;
    }

    compositions.push([ symbolCodes ]);
  }.bind(this));
  ...
  while (++i < n) {
    ...
    combinations.forEach(function(symbolCodes) {
      ...
      compositions.push([].concat(previousFirstResults[i], [ symbolCodes ]));
    }.bind(this));
  }
