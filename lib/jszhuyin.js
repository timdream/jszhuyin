...
(function(factory) {
  if (typeof module === 'object' && module.exports) {
    ...
    factory(module.exports, {
      BopomofoEncoder: require('./bopomofo_encoder.js'),
      JSZhuyinDataPackStorage: require('./storage.js').JSZhuyinDataPackStorage,
      CacheStore: require('./storage.js').CacheStore
    });
  } else if (typeof self === 'object') {
    // Window or WorkerGlobalScope
    if (typeof self.BopomofoEncoder === 'undefined' ||
      typeof self.JSZhuyinDataPackStorage === 'undefined' ||
      typeof self.CacheStore === 'undefined') {
 ...
 */
var ActionQueue = exports.ActionQueue = function ActionQueue() {
  this.pendingActions = [];
  this.waiting = false;
};
/**
...
ActionQueue.prototype.done = function aq_done() {
  if (!this.waiting) {
    throw new Error('ActionQueue: ' +
      'Calling queue.done() when we are not waiting.');
  }
...
var JSZhuyinQueryData = function(symbols, longestPhraseLength) {
  ...
  this._trimmedFromEndSymbols = undefined;
  this._trimmedSymbolsCombinations = undefined;
  this._trimmedFromEndSymbolsCombinations = undefined;
};
...
JSZhuyinQueryData.prototype.getTrimmedFromEndSymbols = function() {
  ...
  return (this._trimmedFromEndSymbols = BopomofoEncoder.trimToLengthFromEnd(
    this.symbols, this.longestPhraseLength));
};
...
JSZhuyinQueryData.prototype.getTrimmedFromEndSymbolsCombinations = function() {
  ...
  return (this._trimmedFromEndSymbolsCombinations =
    BopomofoEncoder.getSymbolCombinations(this.getTrimmedFromEndSymbols()));
};
...
function(expendedEncodedSounds, previousFirstResults) {
  ...
  var composedResultDataArr = previousFirstResults
    .reduce(function(composedResultDataArr, previousFirstResult, i) {
      ...
      var currentComposedResultDataArr = BopomofoEncoder
        .getSymbolCombinations(
          expendedEncodedSounds.slice(i), this.LONGEST_PHRASE_LENGTH)
        ...
        .map(function(symbolCodesResultData) {
          ...
          if (i !== 0) {
            ...
            composedResultData.str += symbolCodesFirstResult.str;
            composedResultData.score += symbolCodesFirstResult.score;
            composedResultData.index = symbolCodesFirstResult.index;
          } else {
      ...
      if (currentComposedResultDataArr.length === 0) {
        // Make sure we always have at least one full candidate.
        // These composed candidates with symbols will have score=-Infinity.
        // When all entries are of score=-Infinity, the sorting function will
        // sort the last entrie to the top and it will happen to be the must
        // useful result.
        var composedResultData;
        if (previousFirstResult) {
          ...
          var firstSymbols =
            BopomofoEncoder.decode(expendedEncodedSounds.slice(i, 1));
          composedResultData = new JSZhuyinComposedResultData(
            null, [ firstSymbols, firstSymbols.length ]);
        }
        composedResultData.str +=
          BopomofoEncoder.decode(expendedEncodedSounds.slice(i));
        composedResultData.score = -Infinity;

        composedResultDataArr.push(composedResultData);
        return composedResultDataArr;
      } else {
        return composedResultDataArr.concat(currentComposedResultDataArr);
      }
    ...
    .reduce(function(orderedSymbolCodeArrs, symbolCodeDataArr) {
      symbolCodeDataArr.forEach(function(symbolCodeData) {
        if (!orderedSymbolCodeArrs[symbolCodeData[0]]) {
          ...
          var isDuplication = orderedSymbolCodeArrs[symbolCodeData[0]]
            .some(function(arr) {
              return (arr.length === symbolCodeData[1]) &&
                arr.some(function(code, i) {
 ...
 */
var JSZhuyin = exports.JSZhuyin = function JSZhuyin() {
  this.storage = null;
 ...
 * You may set this constant to skip asynchronization load() phase.
 * @type {object}
 */
...
/**
 * Unload JSZhuyin. Queue a task to remove everything from memory.
 * @this   {object}   JSZhuyin instance.
 */
...
/**
 * Unload JSZhuyin in this function loop. Remove everything from memory.
 * @this   {object}   JSZhuyin instance.
 */
...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  // Flatten codes array of possible combinations from the beginning and end
  // for cache cleanning in storage.
  // (Disregard the middle part because they will be backed by composed result
  //  caches.)
      ...
      if (i === 0) {
        this.composedCandidatesBuilder.addToCache(
          ...
          new JSZhuyinComposedResultData(
            result, [ result.str, this.symbols.length ]));
      }
...
function(results, queryData) {
  ...
  results.push([composedResultData.str,
    this.candidateMetadata.saveData(
      this.symbols.length, composedResultData.index)]);
...
JSZhuyin.prototype.suggest = function jz_suggest(reqId) {
  ...
  if (this.confirmedPartIndex === this.candidateMetadata.NULL_ID ||
...
JSZhuyin.prototype.updateCandidates = function jz_updateCandidates(results,
                                                                      reqId) {
  // Suggestion candidates cannot be confirmed as default candidate.
