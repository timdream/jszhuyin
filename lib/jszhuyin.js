...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  // phrases.
  if (storage.get(encodedStr)) {
    data = storage.get(encodedStr).getResults();
  ...
  nextComposition: for (i = 0; i < compositions.length; i++) {
    ...
    for (j = 0; j < composition.length; j++) {
      ...
      if (!storage.get(substr) && n > 1) {
        ...
        var dataFirstResult = storage.get(substr).getFirstResult();
  ...
  while (i--) {
    ...
    if (!storage.get(substr)) {
      continue;
    }

    data = storage.get(substr).getResults();
...
JSZhuyin.prototype.suggest = function jz_suggest(reqId) {
  ...
  results.forEach(function each_suggest(dataPack) {
    var dataPackResults = dataPack.getResults();
