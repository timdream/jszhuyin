 ...
 * @type {number}
 */
JSZhuyin.prototype.MAX_ENCODED_SOUNDS_LENGTH = 8;
/**
...
/**
 * Attempt to find multi-word phrases when checking incomplete sound.
 * Turn this on to access all phrases with incomplete sounds,
 * e.g. ㄓㄨ will match not only 中(ㄓㄨㄥˉ) but also 中(ㄓㄨㄥˉ)午(ㄨˇ).
 * @type {boolean}
 */
JSZhuyin.prototype.SPLIT_SOUND_TO_MATCH_PHRASES = true;
/**
...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  this.storage.setMatchPhrasesFromSingleKey(false);

  // Encode the string into Bopomofo encoded string where
  // one character represents a symbols.
  var encodedStr = BopomofoEncoder.encode(this.symbols);

  if (encodedStr.length > this.MAX_ENCODED_SOUNDS_LENGTH) {
  ...
  var compositions = BopomofoEncoder.getSymbolsCompositions(this.symbols);
  // ==== PART I: PHRASES ====
  ...
  compositions
    ...
    .map(function(composition) {
      ...
      // TODO: convert symbolCodes to array until we have the API fixed.
      var key = String.fromCharCode.apply(String, symbolCodes);
      return storage.getIncompleteMatched(key);
    })
    ...
    .forEach(function(result) {
      results.push([result.str,
        this.candidateMetadata.saveData(encodedStr.length, result.index)]);
    }.bind(this));

  if (results.length) {
    firstMatchedPhrase = results[0];
  }
    ...
    .map(function(composition) {
      var composedResultData = composition
        .map(function(symbolCodes) {
          // TODO: convert symbolCodes to array until we have the API fixed.
          var key = String.fromCharCode.apply(String, symbolCodes);
          return [storage.getIncompleteMatched(key), symbolCodes, key];
        })
        .reduce(function(composedResultData, symbolCodesResultData) {
          ...
          if (!symbolCodesResultData[0]) { // No dataPack returned
            ...
            composedResultData[0] +=
              BopomofoEncoder.decode(symbolCodesResultData[2]);
    ...
    .forEach(function(composedResultData) {
      ...
      results.push([composedResultData[0],
        this.candidateMetadata.saveData(
          encodedStr.length, composedResultData[2])]);
    }.bind(this));
  ...
  while (i--) {
    ...
    for (j = 0; j < data.length; j++) {
      res = [data[j].str,
        this.candidateMetadata.saveData(i, data[j].index)];
  ...
  if (!storage.getIncompleteMatched(encodedStr[0])) {
    res = [BopomofoEncoder.decode(encodedStr[0]),
      this.candidateMetadata.saveData(1, 0)];
    if (!firstMatchedPhrase) {
      firstMatchedPhrase = res;
    }
    results.push(res);
...
JSZhuyin.prototype.confirmCandidate = function jz_confirmCandidate(candidate,
  ...
  this.symbols = BopomofoEncoder.decode(
    BopomofoEncoder.encode(this.symbols).substr(metadata[0]));
  this.updateComposition(reqId);
