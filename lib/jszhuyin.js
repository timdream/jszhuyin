...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  // Prepare |compositions| which contains all possible compositions &
  // combinations of the current symbols string.
  var compositions =
  ...
  // matched candidates.
  // The |compositions| arrays will contains "compositions" consist of exactly
  // one symbol string, and that would fits the definition of
  // |trimmedSymbolsCombinations|, so we don't really need to construct it
  // again.
  // However that only happens if expendedEncodedSounds.length does not exceed
  // LONGEST_PHRASE_LENGTH and |trimmedSymbols| is really not equal to
  // |symbols|; If they are different, |compositions| would have zero
  // composition constructed of one symbol string, as symbol strings cannot
  // exceed the size of LONGEST_PHRASE_LENGTH.
  var trimmedSymbols;
  if (expendedEncodedSounds.length > this.LONGEST_PHRASE_LENGTH) {
    trimmedSymbols =
      BopomofoEncoder.trimToLength(this.symbols, this.LONGEST_PHRASE_LENGTH);
  } else {
    trimmedSymbols = this.symbols;
  }

  var trimmedSymbolsCombinations;
  if (trimmedSymbols !== this.symbols) {
    trimmedSymbolsCombinations =
      BopomofoEncoder.getSymbolCombinations(trimmedSymbols);
  } else {
    trimmedSymbolsCombinations = compositions
  ...
  if (!results.length) {
    results = this._insertFullyMatchingComposedCandidates(
      results, expendedEncodedSounds, compositions);
  }
 ...
 * set so many of the conpositions can hopefully eliminated.
...
function(expendedEncodedSounds) {
  ...
  while (i--) {
    cachedComposedResult =
      this.composedResultCache.get(expendedEncodedSounds.slice(0, i));
 ...
 * @param  {array(array(string, number))} results      Result candidates.
 * @param  {array(number)}                expendedEncodedSounds Symbols.
 * @param  {array(array(array(number)))}  compositions Array of possible
 *                                                     compositions.
 * @return {array(array(string, number))} The result candidates array passed
 *                                        (modified in-place).
 */
JSZhuyin.prototype._insertFullyMatchingComposedCandidates =
function(results, expendedEncodedSounds, compositions) {
  var storage = this.storage;
...
JSZhuyin.prototype._insertTypoHints = function(results, expendedEncodedSounds) {
  ...
  // If the first sound doesn't made up a word, show the symbols.
  var str;
