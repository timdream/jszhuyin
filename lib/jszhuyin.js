...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  var singleCompositions = compositions
  ...
  i = expendedEncodedSounds.length;
  while (i--) {
    ...
    storage.getIncompleteMatched(arr).getResults()
      .forEach(function(result) {
        ...
        if (results.length === 0) {
          ...
          results.push([
            result.str +
              BopomofoEncoder.decode(expendedEncodedSounds.slice(arr.length)),
  ...
  var supersetCodes =
    singleCompositions.reduce(function(supersetCodes, composition) {
      var c = composition[0];
      if (c instanceof JSZhuyinCachedComposedResult) {
        return supersetCodes;
      }
      return supersetCodes.concat(c);
    }, /* supersetCodes */ []);
