...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  this.storage.setMatchPhrasesFromSingleKey(this.SPLIT_SOUND_TO_MATCH_PHRASES);
  ...
  var encodedStrOriginal = BopomofoEncoder.encode(this.symbols);
  ...
  if (storage.getIncompleteMatched(encodedStr)) {
    ...
    for (i = 0; i < data.length; i++) {
      res = [data[i].str,
             this.candidateMetadata.saveData(encodedStr.length, data[i].index)];
  ...
  nextComposition: for (i = 0; i < compositions.length; i++) {
    ...
    for (j = 0; j < composition.length; j++) {
      ...
      if (!storage.getIncompleteMatched(substr) && n > 1) {
        ...
        // a single sound to show the typo.
        composedResult +=
        ...
        lastPartIndex = dataFirstResult.index;
      }
      start += n;
    }
    ...
    if (!found) {
      composedResults.push(
        [composedResult, composedResultScore, lastPartIndex]);
    }
  }
  // Sort the results.
  ...
  for (i = 0; i < composedResults.length; i++) {
    ...
    results.push([composedResults[i][0],
      this.candidateMetadata.saveData(
        encodedStr.length, composedResults[i][2])]);
  }
  ...
  if (!storage.getIncompleteMatched(encodedStr[0])) {
    res = [BopomofoEncoder.decode(encodedStrOriginal[0]),
