 ...
 * @type {number}
 */
JSZhuyin.prototype.IDB_VERSION = 6;
/**
 ...
 * @type {function}
 */
JSZhuyin.prototype.onchunkload = null;
/**
...
JSZhuyin.prototype.handleKeyEvent = function jz_handleKeyEvent(code, reqId) {
  ...
  if (BopomofoEncoder.isBopomofoSymbol(code)) {
    // Abort the current query if there is one.
    this.abortDatabaseTxn();
...
JSZhuyin.prototype.load = function jz_load() {
  ...
  storage.DATA_URL = '../data/database.data';
...
*/
  ...
  storage.onchunkload = this.onchunkload;
  storage.onpopulated = this.onpopulated;
...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  if (count === 0) {
    ...
    this.finishQuery(encodedStr, encodedStrOriginal, compositions, reqId);

    return;
  }

  var txn = this.currentTxn = this.storage.getTxn();
  var abortable = !!txn;
  var self = this;
  keysToGet.forEach(function getTermFromStorage(key) {
    self.storage.get(key, function gotTerm(result) {
      if (result) {
        self.cache.add(key, new JSZhuyinDataPack(result));
      }
      ...
      if (abortable && !self.currentTxn) {
        // Aborted. Simply exit from here and not going to finishQuery().
      ...
      self.finishQuery(encodedStr, encodedStrOriginal,
                       compositions, reqId);
    }, txn);
  });
};
/**
 * Abort the current database transitions. This cause the query stop immidently
 * and not return the new candidates.
 */
JSZhuyin.prototype.abortDatabaseTxn = function jz_abortDatabaseTxn() {
  if (!this.currentTxn)
    return;
 ...
 * @param  {string} encodedStrOriginal  The encoded Bopomofo syllables, without
 *                                      adding tones for database lookup.
...
JSZhuyin.prototype.suggest = function jz_suggest(reqId) {
  ...
  this.storage.getRange(this.confirmedEncodedStr, function gotSuggests(result) {
    if ((abortable && !self.currentTxn) || !result) {
    ...
    result.forEach(function each_suggest(obj) {
      var dataPackResults =
        (new JSZhuyinDataPack(obj['values'] || obj)).getResults();
      dataPackResults.forEach(function each_result(dataPackResult) {
        ...
        // Alternatively, we could create alternative index the values
        // in IndexedDB that use the characters as keys, however that will make
        // consume even more disk space.
        if (dataPackResult.str.substr(0, confirmedCharactersLength) !==
            self.confirmedCharacters)
          return;
    ...
    self.updateCandidates(candidates);
    self.sendActionHandled(reqId);
    self.queue.done();
  }, txn);
};
