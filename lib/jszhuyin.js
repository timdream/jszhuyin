...
var JSZhuyin = function JSZhuyin() {
  this.JSON_FILES = ['words.json', 'phrases.json', 'more.json',
                     'shortcuts.json', 'shortcuts-more.json'];
  this.storage = null;
 ...
 * @type {number}
 */
JSZhuyin.prototype.IDB_VERSION = 5;
/**
...
/**
 * Abort the current query. This cause the query stop immidently
 * and not return the new candidates.
 */
JSZhuyin.prototype.abortQuery = function jz_abortQuery() {
  if (!this.currentTxn)
    return;
...
JSZhuyin.prototype.finishQuery = function jz_finishQuery(encodedStr,
  ...
  if (!cache.get(encodedStr[0])) {
    var res = [BopomofoEncoder.decode(encodedStrOriginal[0]), 1];
    if (!firstMatchedPhrase) {
      firstMatchedPhrase = res;
    }
    results.push(res);
