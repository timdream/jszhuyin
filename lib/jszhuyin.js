...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  var arr, key, symbolLength;
  var i = expendedEncodedSounds.length;
  while (i--) {
    arr = expendedEncodedSounds.slice(0, i);
    // TODO: convert symbolCodes to array until we have the API fixed.
    key = String.fromCharCode.apply(String, arr);
    symbolLength = BopomofoEncoder.decode(key).length;
    if (!key.length || !storage.getIncompleteMatched(key)) {
      continue;
    }

    storage.getIncompleteMatched(key).getResults()
      .forEach(function(result) {
        var isDuplication = results.some(function(previousResult) {
  ...
  // show the symbols.
  // TODO: convert symbolCodes to array until we have the API fixed.
  key = String.fromCharCode(expendedEncodedSounds[0]);
  if (BopomofoEncoder.isCompleted(expendedEncodedSounds[0]) &&
      !storage.getIncompleteMatched(key)) {
    var str = BopomofoEncoder.decode(key);
