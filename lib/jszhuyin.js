...
var JSZhuyinCachedResultBase = function JSZhuyinCachedResultBase() {
};
var JSZhuyinCachedResult =
function JSZhuyinCachedResult(data) {
  this.data = data;
};
JSZhuyinCachedResult.prototype =
  Object.create(JSZhuyinCachedResultBase.prototype);
var JSZhuyinCachedNullResult = function JSZhuyinCachedNullResult() {
};
JSZhuyinCachedNullResult.prototype =
  Object.create(JSZhuyinCachedResultBase.prototype);
...
function(expendedEncodedSounds, data) {
  var cachedComposedResult = (data !== null) ?
    new JSZhuyinCachedResult(data) : new JSZhuyinCachedNullResult();
  this.cache.add(expendedEncodedSounds, cachedComposedResult);
...
function(expendedEncodedSounds) {
  var compositions = this._getCompositions(expendedEncodedSounds);
  var composedResultData = this._getFirstComposedResult(compositions);
  this.addToCache(expendedEncodedSounds, composedResultData);

  return composedResultData;
};
JSZhuyinComposedCandidatesBuilder.prototype._getCompositions =
function(expendedEncodedSounds) {
  ...
  while (++i < n) {
    ...
    cachedComposedResult =
      this.cache.get(slicedExpendedEncodedSounds);
    if (!cachedComposedResult) {
      cachedComposedResult = this._calculateCache(
...
JSZhuyinComposedCandidatesBuilder.prototype._calculateCache =
function(expendedEncodedSounds, startsWithCompositions) {
  var compositions = BopomofoEncoder.getSymbolsCompositions(
    expendedEncodedSounds, this.LONGEST_PHRASE_LENGTH, startsWithCompositions);

  var composedResultData = this._getFirstComposedResult(compositions);
  return this.addToCache(expendedEncodedSounds, composedResultData);
};
    ...
    .map(function(composition) {
      var composedResultData = composition
        .map(function(symbolCodes) {
          if (symbolCodes instanceof JSZhuyinCachedResultBase) {
            return [symbolCodes];
          }
        ...
        .reduce(function(composedResultData, symbolCodesResultData, i) {
          ...
          // No dataPack returned
          if (!symbolCodesResultData[0]) {
            return null;
          }

          // Returned object is a null result placeholder
          if (symbolCodesResultData[0] instanceof JSZhuyinCachedNullResult) {
            return null;
          }

          if (symbolCodesResultData[0] instanceof JSZhuyinCachedResult) {
            if (i !== 0) {
              throw new Error('JSZhuyinComposedCandidatesBuilder: ' +
                'Expects JSZhuyinCachedResult at the 0th position.');
            }

            // Reconstruct the array so we won't modified the returned result
            // of this instance.
            return [].concat(symbolCodesResultData[0].data);
          }
...
function(combinations, insertFullyMatched) {
  var storage = this.storage;

  var dataPackResultDataArr = combinations
    .map(function(symbolCodes) {
...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  results = this._insertPartialMatchingCandidates(
    results, trimmedSymbolsCombinations, (trimmedSymbols !== this.symbols));
  ...
  this.composedCandidatesBuilder.cleanupCache(supersetCodes);
...
function(results, expendedEncodedSounds) {
  ...
  var composedResultData = this.composedCandidatesBuilder
    .getComposedCandidates(expendedEncodedSounds);
...
function(results, combinations, insertFullyMatched) {
  var dataPackResultDataArr = this.partiallyMatchedCandidatesBuilder
    .getCandidates(combinations, insertFullyMatched);
