...
var JSZhuyinCachedResult =
function JSZhuyinCachedResult(result, overflowCandidateData) {
  this._result = result;
  this._overflowCandidateData = overflowCandidateData;
};
JSZhuyinCachedResult.prototype.getFirstResult = function() {
  return this._result;
};
JSZhuyinCachedResult.prototype.getOverflowCandidateData = function() {
  return this._overflowCandidateData;
};
 ...
 * @type {number}
 */
JSZhuyin.prototype.MAX_SOUNDS_LENGTH = 18;
/**
...
JSZhuyin.prototype.load = function jz_load(data) {
  ...
  this.composedResultCache = new CacheStore();
...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  // First, construct |expendedEncodedSounds| to evaluate MAX_SOUNDS_LENGTH.
  ...
  if (expendedEncodedSounds.length > this.MAX_SOUNDS_LENGTH) {
    ...
    expendedEncodedSounds = BopomofoEncoder.encodeExpended(this.symbols);
    this.updateComposition(reqId);
  }

  // Prepare everything needed for running our internal functions to construct
  // the query results.

  // Prepare |trimmedSymbolsCombinations| for creating list of partially
  // matched candidates, and also fully matching candidates, if applies.
  var trimmedSymbols =
    BopomofoEncoder.trimToLength(this.symbols, this.LONGEST_PHRASE_LENGTH);
  var trimmedSymbolsCombinations =
    BopomofoEncoder.getSymbolCombinations(trimmedSymbols);
  ...
  if (trimmedSymbols === this.symbols) {
    results = this._insertFullyMatchingCandidates(
      results, expendedEncodedSounds, trimmedSymbolsCombinations);
  }
  ...
  if (!results.length) {
    results = this._insertFullyMatchingComposedCandidates(
      results, expendedEncodedSounds);
  }

  // Inserting a placeholder cache here so we won't try to cacluate compose
  // result from the current expendedEncodedSounds again.
  if (!results.length) {
    this.composedResultCache.add(
      expendedEncodedSounds,
      new JSZhuyinCachedResult({
        str: '', index: 0, score: -Infinity
      }, [ '', 0 ]));
  }

  // ==== PART III: PHRASES THAT MATCHES SYMBOLS PARTIALLY ====
  results =
    this._insertPartialMatchingCandidates(results, trimmedSymbolsCombinations);

  // ==== PART IV: UNFORTUNATE TYPO ====
  // Lastly, append the symbols as the last candidate to give out a proper
  // feedback (if applicable).
  results = this._insertTypoHints(results, expendedEncodedSounds);

  this.updateCandidates(results);
  this.sendActionHandled(reqId);
  this.queue.done();

  // Flatten codes array of all single compositions for cache cleanning.
  ...
  // Remove cache entires that will not be helpful to us in the future.
  this.composedResultCache.cleanup(supersetCodes);
  // Ask storage to clean-up cache too.
  storage.cleanupCache(supersetCodes);
};
/**
 * Get all possible symbol conpositions and combinations, but with cache
 * set so many of the compositions can hopefully eliminated.
 * @param   {array(number)}  expendedEncodedSounds Symbols.
 * @returns {array(array(array(number)))}  Array of possible compositions.
 */
JSZhuyin.prototype.getSymbolsCompositionsWithCache =
function(expendedEncodedSounds) {
  // First we construct |startsWithCompositions| array which will contains
  // JSZhuyinCachedResult instances from the cache.
  // With them we would be able to greatly eliminate number of componsitions
  // to query and calculate.
  var startsWithCompositions = [];
  var cachedComposedResult;
  var i = 0;
  ...
  while (++i < n) {
    ...
    cachedComposedResult =
      this.composedResultCache.get(slicedExpendedEncodedSounds);
    if (!cachedComposedResult) {
      cachedComposedResult = this._calculateComposedResultCache(
        slicedExpendedEncodedSounds, startsWithCompositions);
    }
    if (cachedComposedResult) {
      startsWithCompositions.push([i, cachedComposedResult]);
    }
  }

  // Return |compositions| to the caller.
  return BopomofoEncoder.getSymbolsCompositions(
    expendedEncodedSounds, this.LONGEST_PHRASE_LENGTH, startsWithCompositions);
};
JSZhuyin.prototype._calculateComposedResultCache =
function(expendedEncodedSounds, startsWithCompositions) {
  var compositions = BopomofoEncoder.getSymbolsCompositions(
    expendedEncodedSounds, this.LONGEST_PHRASE_LENGTH, startsWithCompositions);

  var composedResultDataArr = this._getComposedResults(compositions);
  var composedResultData = composedResultDataArr[0];
  var cachedComposedResult;

  if (composedResultDataArr[0]) {
    cachedComposedResult = new JSZhuyinCachedResult({
      str: composedResultData[0],
      index: composedResultData[2],
      score: composedResultData[1]
    }, [ composedResultData[3][0], composedResultData[3][1] ]);
  } else {
    cachedComposedResult = new JSZhuyinCachedResult({
        str: '', index: 0, score: -Infinity
      }, [ '', 0 ]);
  ...
  this.composedResultCache.add(expendedEncodedSounds, cachedComposedResult);

  return cachedComposedResult;
};
/**
    ...
    .forEach(function(result, i) {
      ...
      if (i === 0) {
        this.composedResultCache.add(
          expendedEncodedSounds,
          new JSZhuyinCachedResult(
            { str: result.str, index: result.index, score: result.score },
...
/**
 * For a given compositions, get an array of matched composed results from
 * database.
 * @param  {array(array(number))} compositions Array of possible compositions.
 * @return {array([string, number, number, [string, number]])}
 *         The array of composedResultData.
 */
JSZhuyin.prototype._getComposedResults = function(compositions) {
  var storage = this.storage;

  var composedResultDataArr = compositions
    .map(function(composition) {
      var composedResultData = composition
        .map(function(symbolCodes) {
          if (symbolCodes instanceof JSZhuyinCachedResult) {
            return [symbolCodes];
          }

          return [storage.getIncompleteMatched(symbolCodes), symbolCodes];
        }.bind(this))
        .reduce(function(composedResultData, symbolCodesResultData, i) {
          // Don't do anything if the composedResultData passed is already
          // null.
          ...
          if (i === 0) {
            if (symbolCodesResultData[0] instanceof JSZhuyinCachedResult) {
              composedResultData[3] =
                symbolCodesResultData[0].getOverflowCandidateData();
            } else {
              composedResultData[3] = [firstResult.str,
...
function(results, expendedEncodedSounds) {
  var compositions =
    this.getSymbolsCompositionsWithCache(expendedEncodedSounds);

  this._getComposedResults(compositions)
    .some(function(composedResultData) {
      if (composedResultData[1] === -Infinity) {
        return false;
      }

      if (!this.overflowCandidateString && composedResultData[3]) {
      ...
      // Stop the recursion so we would only push the first ranked combined
      // result.
      return true;
    }.bind(this));
