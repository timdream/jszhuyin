...
var JSZhuyinCachedComposedResult =
function JSZhuyinCachedComposedResult(result) {
  this._result = result;
};
JSZhuyinCachedComposedResult.prototype.getFirstResult = function() {
  return this._result;
};
...
function(expendedEncodedSounds) {
  ...
  // JSZhuyinCachedComposedResult instances from the cache.
    ...
    .forEach(function(result, i) {
      ...
      if (i === 0) {
        this.composedResultCache.add(
          ...
          new JSZhuyinCachedComposedResult({
            str: result.str,
            index: result.index,
            score: result.score
          }));
  ...
  compositions
    .map(function(composition) {
      var composedResultData = composition
        .map(function(symbolCodes) {
          if (symbolCodes instanceof JSZhuyinCachedComposedResult) {
            return [symbolCodes];
          }
        ...
        .reduce(function(composedResultData, symbolCodesResultData, i) {
          ...
          if (!symbolCodesResultData[0]) { // No dataPack returned
            ...
            var str = BopomofoEncoder.decode(symbolCodesResultData[1]);

            // Don't discard the result so the user would find out the typo.
            composedResultData[0] += str;
            composedResultData[1] += -Infinity;
            composedResultData[2] = 0;
            if (i === 0) {
              composedResultData[3] = [str, str.length];
            }
          ...
          if (i === 0 && symbolCodesResultData[1]) {
