...
JSZhuyin.prototype.query = function jz_query(reqId) {
  ...
  var compositions = BopomofoEncoder.getSymbolsCompositions(
      expendedEncodedSounds, this.LONGEST_PHRASE_LENGTH,
      startsWithCompositions);
  var singleCompositionCodes = compositions
  ...
  results = this._insertPartialMatchingCandidates(
    results, expendedEncodedSounds, singleCompositionCodes);
  ...
  var supersetCodes =
    singleCompositionCodes.reduce(function(supersetCodes, symbolCodes) {
      if (symbolCodes instanceof JSZhuyinCachedComposedResult) {
        return supersetCodes;
      }
 ...
 * @param  {array(array(number))}         singleCompositionCodes Array
 *                                                               of possible
 *                                                               combinations.
...
function(results, expendedEncodedSounds, singleCompositionCodes) {
  var storage = this.storage;

  if (expendedEncodedSounds.length > this.LONGEST_PHRASE_LENGTH) {
    var symbols = BopomofoEncoder.trimToLength(
      this.symbols, this.LONGEST_PHRASE_LENGTH);
    // XXX: Improve this; we are discarding stuff that takes time to calculate.
    singleCompositionCodes = BopomofoEncoder.getSymbolsCompositions(symbols)
      .filter(function(composition) {
        return (composition.length === 1);
      })
      .map(function(composition) {
