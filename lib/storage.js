 ...
 * @type {object}
 */
IndexedDBStorage.prototype.IDB = window.indexedDB ||
  window.webkitIndexedDB || window.mozIndexedDB || window.msIndexedDB;
/**
 * Reference to IDBDatabase, removing vendor prefix.
 * @type {object}
 */
IndexedDBStorage.prototype.IDBDatabase = window.IDBDatabase ||
  window.webkitIDBDatabase || window.msIDBDatabase;
/**
 * Reference to IDBKeyRange, removing vendor prefix.
 * @type {object}
 */
IndexedDBStorage.prototype.IDBKeyRange = window.IDBKeyRange ||
  window.webkitIDBKeyRange || window.msIDBKeyRange;
/**
 * Reference to IDBIndex, removing vendor prefix.
 * @type {object}
 */
IndexedDBStorage.prototype.IDBIndex = window.IDBIndex ||
  window.webkitIDBIndex || window.msIDBIndex;
/**
...
IndexedDBStorage.prototype.putChunk = function idb_putChunk(chunk, callback) {
  ...
  var putSmallChunk = function putSmallChunk() {
    ...
    txn.onerror = function idb_putchunk_error() {
      ...
      throw 'putChunk() failed. It is most likely because object of ' +
        'the same key has been there already.';
    };

    while (i < MAX && keys.length) {
      var key = keys.shift();
      store.add({ 'key': key, 'values': data[key] });
      i++;
    }
...
HybirdStorage.prototype.loadJSONStorage =
  function hs_loadJSONStorage(populateIdb) {
    ...
    if (populateIdb) {
      ...
      jsonStorage.onerror = this.onerror;
