  ...
  while ((code = keyArray.shift()) !== undefined) {
    ...
    for (i = 0; i < addresses.length; i++) {
      ...
      if (!this._interchangeablePairs &&
        ...
        a = this.searchBlock(code, addresses[i]);
        if (a !== -1) {
          ak = ak.concat([a]);
        }
      ...
      results = (this._matchPhrasesFromSingleKey) ?
        this._getIncompleteMatchedCodesInBlock(code, addresses[i]) :
        this._getIncompleteMatchedSingleCodesInBlock(code, addresses[i]);

      ak = ak.concat(results);
    }
    addresses = ak;
  }
...
function jdps__getIncompleteMatchedCodesInBlock(code, byteOffset) {
  var addresses = [];
  var codeSplits = BopomofoEncoder.split(code);
  codeSplits.forEach(function(codes) {
    var currentAddresses = [byteOffset];
    codes.forEach(function(code) {
      var currentCodeAddresses = [];
      currentAddresses.forEach(function(address) {
        currentCodeAddresses = currentCodeAddresses.concat(
          this._getIncompleteMatchedSingleCodesInBlock(code, address));
      }, this);
      currentAddresses = currentCodeAddresses;
    }, this);
    addresses = addresses.concat(currentAddresses);
  }, this);
