...
(function(factory) {
  if (typeof module === 'object' && module.exports) {
    ...
    factory(module.exports, {
      JSZhuyinDataPack:
        require('./jszhuyin_data_pack.js').JSZhuyinDataPack,
      JSZhuyinDataPackCollection:
        require('./jszhuyin_data_pack.js').JSZhuyinDataPackCollection,
      BopomofoEncoder: require('./bopomofo_encoder.js')
    });
  } else if (typeof self === 'object') {
    // Window or WorkerGlobalScope
    if (typeof self.JSZhuyinDataPack === 'undefined' ||
      typeof self.JSZhuyinDataPackCollection === 'undefined' ||
      typeof self.BopomofoEncoder === 'undefined') {
 ...
 */
var CacheStore = exports.CacheStore = function CacheStore() {
 ...
 * @constructor
 */
var BinStorage = exports.BinStorage = function BinStorage() {
  this.loaded = false;
  this._bin = undefined;
};
/**
...
var JSZhuyinDataPackStorage = exports.JSZhuyinDataPackStorage = function() {
...
JSZhuyinDataPackStorage.prototype.setInterchangeablePairs = function(str) {
  ...
  var arr = encodedSounds
    .reduce(function(arr, code, i, sounds) {
      if (i % 2) {
        return arr;
      }

      arr.push([code, sounds[i + 1]]);
      return arr;
    }, []);
...
JSZhuyinDataPackStorage.prototype.getIncompleteMatched = function(codes) {
  ...
  var addresses = codes
    .reduce(function(addresses, code) {
      var codesToLookup = this._getInterchangeableCodes(code)
        .reduce(function(arr, c) {
          return arr.concat(Array.from(
            BopomofoEncoder.getMatchedCompleteEncodedSounds(c)));
        }, []);
      ...
      return addresses.reduce(function(codeAddresses, address) {
        var results = codesToLookup
          .map(function(c) {
            return this.searchBlock(c, address);
          }.bind(this))
          .filter(function(a) {
            return (a !== -1);
          });
...
function jdps__getIncompleteMatchedSingleCodesInBlock(code, byteOffset) {
  ...
  var isIncompletionOfCode =
    BopomofoEncoder.createCompletionComparisonFunction(code);
 ...
 * @param  {String}  phraseMaxLength Max length of the phrase to match.
 * @param  {Boolean} group           Group encoded sounds in phrases.
 * @return {array(array(number))|
 *          array(number)}           Encoded sounds of Bopomofo, group in
 *                                   phrases. 0 for sounds can't be found.
 */
JSZhuyinDataPackStorage.prototype.reverseGet = function(str,
  ...
  // Get all the dataPacks in the entire database.
  var dataPacks = [];
  var getDataPacks = function(byteOffset, keys) {
    var view = new DataView(bin, byteOffset);
    var length = view.getUint16(0, true);
    var contentLength = view.getUint16(2, true);
    ...
    if (contentLength !== 0) {
      ...
      var dataPack = Object.create(JSZhuyinDataPack.prototype);
      JSZhuyinDataPack.apply(dataPack, result);
      if (!dataPacks[keys.length]) {
        dataPacks[keys.length] = [];
      }
      dataPacks[keys.length].push([dataPack, keys]);
    }

    if ((keys.length + 1) > phraseMaxLength) {
      return;
    }

    for (var i = 0; i < length; i++) {
      getDataPacks(
        addressBlockView.getUint32(i << 2, true),
        [].concat(keys, keyBlockView.getUint16(i << 1, true)));
    }
  }.bind(this);
  getDataPacks(0, []);
...
/**
 * Internal method to expand the code to array of possible codes based on
 * the interchangeable pairs set.
 * @param  {number} code    One sound represented in number
 * @return {array(number)}  Array of codes
 */
JSZhuyinDataPackStorage.prototype._getInterchangeableCodes = function(code) {
  if (!this._interchangeablePairs) {
    return [code];
  }

  return this._interchangeablePairsArr
    .reduce(function(codes, pair) {
      codes.forEach(function(code) {
        var c = BopomofoEncoder.replace(code, pair[0], pair[1]);
        if (code !== c) {
          codes.push(c);
        }
        c = BopomofoEncoder.replace(code, pair[1], pair[0]);
        if (code !== c) {
          codes.push(c);
        }
      });
