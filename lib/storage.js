...
/* global JSZhuyinDataPack, BopomofoEncoder */
...
JSZhuyinDataPackStorage.prototype.getIncompleteMatched = function(key) {
  ...
  while ((code = keyArray.shift()) !== undefined) {
    bf = [];
    for (i = 0; i < byteOffsets.length; i++) {
      bf = bf.concat(
        this._getIncompleteMatchedCodeInBlock(code, byteOffsets[i]));
    }
    byteOffsets = bf;
  }

  return byteOffsets
    .map(function(byteOffset) {
      return this._getBlockContent(byteOffset);
    }, this)
 ...
 * @type {[type]}
 */
JSZhuyinDataPackStorage.prototype._getIncompleteMatchedCodeInBlock =
function jdps__getIncompleteMatchedCodeInBlock(code, byteOffset) {
  var bin = this._bin;
  var view = new DataView(bin, byteOffset);
  var length = view.getUint16(0, true);
  var contentLength = view.getUint16(2, true);
  ...
  for (var i = 0; i < length; i++) {
    if (BopomofoEncoder.isIncompletionOf(
        code, keyBlockView.getUint16(i << 1, true))) {
