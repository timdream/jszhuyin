...
var JSZhuyinDataPackStorage = function() {
  ...
  this._matchPhrasesFromSingleKey = true;
};
...
/**
 * When attempt to find results with getIncompleteMatched(), matches phrases
 * from single keys.
 * e.g. encoded sound represents ㄓㄨ will match not only 中(ㄓㄨㄥˉ),
 * but also 中(ㄓㄨㄥˉ)午(ㄨˇ).
 * @param {boolean} m True if match phrases.
 */
JSZhuyinDataPackStorage.prototype.setMatchPhrasesFromSingleKey = function(m) {
  m = !!m;
  if (this._matchPhrasesFromSingleKey === m) {
    return;
  }

  this.cache.cleanup('');
  this._matchPhrasesFromSingleKey = m;
};
/**
...
JSZhuyinDataPackStorage.prototype.getIncompleteMatched = function(key) {
  ...
  // First, convert key string into an array of code to lookup.
  var codes = key.split('')
    .map(function str2CharCode(char) {
      return char.charCodeAt(0);
    });
  ...
  var addresses = codes
    .reduce(function(addresses, code) {
      ...
      return addresses.reduce(function(codeAddresses, address) {
        ...
        // or not.
        var results = (this._matchPhrasesFromSingleKey) ?
          this._getIncompleteMatchedCodesInBlock(code, address) :
 ...
 * This method will split the code into multiple encoded sounds if possible.
 * @param  {number}   code            code of the character.
 * @param  {numbber}  byteOffset      Byte offset of the block.
...
function jdps__getIncompleteMatchedCodesInBlock(code, byteOffset) {
  // This is done by attempt to pass the code into the non-spliting lookup
  // method after splitting it.
  // The outmost reduce() call reduces to all addresses matches to all the
  // split compositions.
  return BopomofoEncoder.split(code).reduce(function(addresses, codes) {
    // Each of the split compositions ("codes") should be matches seperately,
    // and reduces to addresses match this split compositions.
    // addresses is the reduced result of checking the codes in sequences,
    // start by looking at matched keys at table the given byteOffset.
    return addresses.concat(codes.reduce(function(codesAddresses, code) {
      // Thie reduces to the every addresses that matches to the code, starting
      // from empty array.
      // Return back the reduced result and replaces the previous array of
      // addresses.
      return codesAddresses.reduce(function(currentCodeAddresses, address) {
        return currentCodeAddresses.concat(
          this._getIncompleteMatchedSingleCodesInBlock(code, address));
      }.bind(this), /* currentCodeAddresses */ []);
    }.bind(this), /* codesAddresses */ [ byteOffset ]));
  }.bind(this), /* addresses */ []);
};
/**
 * Internal method to get all the addresses point from a given block that
...
JSZhuyinDataPackStorage.prototype.reverseGet = function(str) {
  ...
  var resBuf = new Uint16Array(str.length);
  ...
  for (var i = 0; i < length; i++) {
    ...
    for (j = 0; j < str.length; j++) {
      if (dataPack.getResultsBeginsWith(str[j]).length) {
        resBuf[j] = keyBlockView.getUint16(i << 1, true);
      }
    }
  }

  return Array.prototype.map.call(resBuf, function(c) {
