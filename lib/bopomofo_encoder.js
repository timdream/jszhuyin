...
var BopomofoEncoder = {
  ...
  // expended as seperate symbols. Useful for getSymbolsCompositions().
  encodeExpended: function(symbols) {
    ...
    while (i--) {
      encodedSymbolCode = this.encode(symbols[i]).charCodeAt(0);
  ...
  getSymbolsCompositions: function(symbols) {
    // Construct an encodeExpended encoded sounds
    var expendedEncodedSounds = this.encodeExpended(symbols);
    ...
    var x, a, j, k, p, filled2, filled3, code;
    x = 1 << n - 1;
    while (x--) {
      k = 0;
      p = 0;
      a = [ [ expendedEncodedSounds[0] ] ];
      j = 0;
      while (n - 1 > j) {
        if (x & (1 << j)) {
          code = expendedEncodedSounds[++j];
          // If the previous symbol is a complete one of this is a completed
          // one, they must not occupy the same k-th place.
          ...
          filled2 = !!(a[p][k] & this.BOPOMOFO_GROUP_2_BITMASK);
          filled3 = !!(a[p][k] & this.BOPOMOFO_GROUP_3_BITMASK);

          // If the previous one contain the same or the lower place of symbol,
          // the new symbol must not occupy the same k-th place.
          if ((code & this.BOPOMOFO_GROUP_1_BITMASK) ||
              (code & this.BOPOMOFO_GROUP_2_BITMASK && (filled2 || filled3)) ||
          ...
          // Merge the two symbols into the same k-space.
          a[p][k] |= code;
        } else {
          a.push([ expendedEncodedSounds[++j] ]);
          p++;
          k = 0;
        }
      }
