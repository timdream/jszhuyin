...
var BopomofoEncoder = {
  ...
  getSymbolCombinations: function(symbols) {
    ...
    var combinations = [ [ expendedEncodedSounds[0] ] ];
   ...
   *                                                  returned from
   *                                                  encodeExpended.
   * @param {number}                    longestLength Discard the composition
   *                                                  if it consist more than
   *                                                  # symbols.
   * @param {array(array(number, any))} startsWith    Take previous known top
   *                                                  compositions and
   *                                                  eliminate known lower
   *                                                  ranked compositions.
   * @return {array(array(array(number)))}        Compositions of the string.
   *                                              Read test cases to understand
   *                                              this.
   */
  getSymbolsCompositions: function(symbols, longestLength, startsWith) {
    // Construct an encodeExpended encoded sounds array,
    // or use the input as such.
    var expendedEncodedSounds =
      (typeof symbols === 'string') ? this.encodeExpended(symbols) : symbols;
    ...
    var i;
    var shortestPossibleLengths = [];
    for (i = 0; i < n; i++) {
      // FIXME?: encode(decode())
      shortestPossibleLengths.push(
        this.encode(this.decode(expendedEncodedSounds.slice(i + 1))).length);
    }
    ...
    nextX: while (x--) {
      ...
      // See if there is a startsWith array available for us to skip this
      // x-th series.
      for (i = 0; i < startsWith.length; i++) {
        // Only handle x-th serie w/ a boundary at the given position.
        if ((x & (1 << (startsWith[i][0] - 1))) !== 0) {
          continue;
        }
        // Skip the x-th series entirely if we know the remaining compositions
        // will only result duplications, since we have replaced the part up to
        // |startsWith[i][0] - 1|) to |startsWith[i][1]|.
        // We can simply figure out that by probing if there is any higher bits
        // in |x|.
        remainingX = !!(x >> (startsWith[i][0] - 1));
        if (startsWithIsDuplicated[i] &&
            startsWithIsDuplicated[i].indexOf(remainingX) !== -1) {
          continue nextX;
        }
        if (!startsWithIsDuplicated[i]) {
          startsWithIsDuplicated[i] = [];
        }
        startsWithIsDuplicated[i].push(remainingX);

        // This x-th series should start with our partial composition of
        // |startsWith[i][1]|.
        aArr = [ [ startsWith[i][1] ] ];
        j = startsWith[i][0] - 1;
      ...
      // What's the shortest possible remaining composition length
      // for this x-th series? We should be able to skip it early instead of
      // getting a bunch of null in |aArr|.
      if ((x >> j) === 0 &&
          shortestPossibleLengths[j] > longestLength) {
        continue nextX;
      }

      // Walk the entire expendedEncodedSounds from |j|.
      while (n - 1 > j) {
        // If this bitwise statement is true, we should NOT insert a "boundary"
        // between symbols here.
        if (x & (1 << j)) {
          // |code| should be merge into the current a[p][k] if applicable,
          // if not, it should remain in the same a[p] and be appended.
          // Even if the |code| is merged, we would still need to create a new
          // entry in the current x-th series (the |aArr| array)
          // just to make sure we query the appended version too.
          code = expendedEncodedSounds[++j];
