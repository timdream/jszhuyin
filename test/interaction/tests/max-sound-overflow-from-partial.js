...
(function(factory) {
  ...
  TaskTest.NAME =
    'Type ㄊㄞˊㄅㄟˇㄅㄟˊㄅㄟˊ,ㄊ, with MAX_SOUNDS_LENGTH=4, ' +
    'overflows one phrase from the composed partial-typo result.';
  TaskTest.prototype = {
    config: {
      MAX_SOUNDS_LENGTH: 4
    },
    tasks: [
      {
        ...
        checkCallbackValues: function(values) {
          assert.strictEqual(values.compositionupdate, 'ㄊㄞˊㄅㄟˇㄅㄟˊㄅㄟˊ');
          assert.deepEqual(values.candidateschange,
            [['台北ㄅㄟˊㄅㄟˊ', this.candidateId++],
             ['台北', this.candidateId++],
      ...
      },
      {
        ...
        checkCallbackValues: function(values) {
          ...
          assert.strictEqual(values.compositionupdate, 'ㄅㄟˊㄅㄟˊㄊ');
          assert.deepEqual(values.candidateschange,
            [['ㄅㄟˊㄅㄟˊ台', this.candidateId++],
